...
if (strlen($schet2)==0){ echo "0".($schetfactura_date->schet_new);} else {echo $schet2;} echo " от ";
if (strlen($data_schet)==0){ echo datetostring($schetfactura_date->date);} else { echo $data_schet;}?>
 ...
 <tr> <td width=2100px> <?php echo $org->svidetelstvo_nds; ?></td></tr>
 <tr>
 <td width=2100px>Тауарлар (жұмыстар, қызметтер) жеткізуге арналған шарт (келісім-шарт)<br/>Договор(контракт) на поставку товаров (работ, услуг) №<u>   <?php if (strlen($schet_dog)==0) { echo $firm->dogovor."   </u> от <u>    ".datetostring($firm->dogovor_date);} else echo $schet_dog?></u></td>
 <td> (3)</td>
 </tr>
 <tr>
...
if (strlen($schet2)==0){ echo "0".($schetfactura_date->id);} else {echo $schet2;} echo " от ";
...
<tr align="center">
	...
	<TD width="400px"><?php if (strlen($electric)=='') {echo "Электроэнергия";} else {echo $electric;} ?> за <?php echo datetostring2($schetfactura_date->date);?> года </td>
...
<table border=0 cellspacing=0 cellpadding=0 width=100%><tr>
		...
		<td >Всего отпущено на сумму (прописью), в KZT: <?php echo num2str($itog->itogo_with_nds+$fine_value);?></td>
		</tr></table></b><br/>
<br />
 <FONT size=7>
 <table width="100%">
 ...
 <td></td>
 <td align="right"><b>БЕРДІ (жеткізушінің жауапты тұлғасы)<br/>ВЫДАЛ (ответственное лицо поставщика) Инженер Иваницкий А.П.</b></td>
  </tr>
 <tr>
 <td> </td>
 <td align= "center">

    ...
    <table width=100%>
        <tr align=center>
            <td width=2100px>
                <b><?php echo(strlen(trim($akt_vypolnenyh_rabot)) == 0 ? "Есеп шот фактура / Счет-фактура" : $akt_vypolnenyh_rabot); ?>
                    № </b> <?php
                if (strlen($schet2) == 0) {
                    echo "0" . ($schetfactura_date->schet_new);
                } else {
                    echo $schet2;
                }
                echo " от ";
        ...
        <tr>
            <td width=2100px>Тауарлар (жұмыстар, қызметтер) жеткізуге арналған шарт (келісім-шарт)<br/>Договор(контракт)
                на поставку товаров (работ, услуг) №<u>   <?php if (strlen($schet_dog) == 0) {
                        echo $firm->dogovor . "   </u> от <u>    " . datetostring($firm->dogovor_date);
                    } else echo $schet_dog ?></u></td>
            <td> (3)</td>
        </tr>
        <tr>
    ...
    <table border="1px" width="100%" align="center">
        ...
        foreach ($s as $s2):
            ...
            <tr align="center">
                ...
                <TD width="400px"><?php if (strlen($electric) == '') {
                        echo "Электроэнергия";
                    } else {
                        echo $electric;
                    } ?> за <?php echo datetostring2($schetfactura_date->date); ?> года
                </td>

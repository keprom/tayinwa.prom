...
//        $this->output->enable_profiler(true);
    ...
    function schetfactura()
    {
        ...
        $data['s'] = $this->db->get("industry.schetfactura")->result();

        $this->db->where('id', $_POST['period_id']);
        $data['dtqr'] = $this->db->get("industry.period")->result();
        ...
        $data['schet_dog'] = $_POST['schet_dog'];
        ...
        $this->load->view("reports/sf", $data);
//		if (!isset($_POST['html']))
//		{
...
//		}
    ...
    function schetoplata()
    {
        ...
        $this->db->where('id', $_POST['firm_id']);
        $data['firm'] = $this->db->get('industry.firm')->row();
        $data['number'] = $_POST['number_schet'] == "" ? $data['schetfactura_date']->schet_new : $_POST['number_schet'];
        $this->db->where('id', $_POST['period_id']);
        $data['period'] = $this->db->get('industry.period')->row();
            ...
            foreach ($nach->result() as $n) {
                dbase_add_record($db2,
                    array(
                        $n->dog,
                        $n->kvt,
                        $n->tarif, $n->beznds, $n->nds, $n->snds, "04" . $n->nomer, $this->d2($n->data), "0" . $n->dog1
            ...
            foreach ($nach->result() as $n) {
                dbase_add_record($db2,
                    array(
                        $this->d2($n->DATA),
                        $n->NAIM,
                        $n->DOG, $n->NACH, $n->NACH_SUM, $n->NACH_NDS, $n->KVT, "0" . $n->NOMERSCHET, $this->d2($n->KONDATA), $n->NPENI
                    ));
            }
